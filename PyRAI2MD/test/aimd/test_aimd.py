...
def TestAIMD():
    ...
    testdir = '%s/results/aimd' % (os.getcwd())
    record = {
        'qm_nac': 'FileNotFound',
        'qm_soc': 'FileNotFound',
        'qm_orb': 'FileNotFound',
    ...
    filepath = './aimd/aimd_data/c2h4.inp'
    if os.path.exists(filepath):
        record['qm_nac'] = filepath

    filepath = './aimd/aimd_data/c2h4.isc.inp'
    if os.path.exists(filepath):
        record['qm_soc'] = filepath

    filepath = './aimd/aimd_data/c2h4.StrOrb'
    if os.path.exists(filepath):
        record['qm_orb'] = filepath

    filepath = './aimd/aimd_data/c2h4.xyz'
    if os.path.exists(filepath):
        record['qm_xyz'] = filepath

    filepath = './aimd/aimd_data/c2h4.velo'
    if os.path.exists(filepath):
        record['qm_velo'] = filepath

    filepath = './aimd/aimd_data/PSB5.inp'
    if os.path.exists(filepath):
        record['qmmm_eg'] = filepath

    filepath = './aimd/aimd_data/PSB5.xyz'
    if os.path.exists(filepath):
        record['qmmm_xyz'] = filepath

    filepath = './aimd/aimd_data/PSB5.StrOrb'
    if os.path.exists(filepath):
        record['qmmm_orb'] = filepath

    filepath = './aimd/aimd_data/PSB5.velo'
    if os.path.exists(filepath):
        record['qmmm_velo'] = filepath

    filepath = './aimd/aimd_data/PSB5.key'
    if os.path.exists(filepath):
        record['qmmm_key'] = filepath

    filepath = './aimd/aimd_data/melacu63.prm'
    if os.path.exists(filepath):
...
def CopyInput(record, testdir):
    if not os.path.exists('%s/fssh' % testdir):
        os.makedirs('%s/fssh' % testdir)

    if not os.path.exists('%s/gsh_soc' % testdir):
        os.makedirs('%s/gsh_soc' % testdir)

    if not os.path.exists('%s/gsh' % testdir):
...
def RunMD(testdir):
    ...
    subprocess.run('pyrai2md fssh', shell=True)
    os.chdir(maindir)
    ...
    subprocess.run('pyrai2md gsh_soc', shell=True)
    os.chdir(maindir)
    ...
    subprocess.run('pyrai2md gsh', shell=True)
    os.chdir(maindir)
