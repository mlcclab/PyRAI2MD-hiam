...
system_rdf = {
    ...
    'save_data': 0,  # save the distance vector, 0 not save, 1 save n * n distances
}
...
system_angle = {
    ...
    'save_data': 0,  # save the angel vector, 0 not save, 1 save a - b and c - b
}
...
def compute_angle(idx, coord, record_coord, box, info):
    ...
    b1 = info['angle']['b1']
    b2 = info['angle']['b2']
    save_data = info['angle']['save_data']
    if skip:
        return '', [], []
    ...
    return output, np.reshape(a_vec, -1), np.reshape(a_vec2, -1)
...
def main():
    ...
    for n, trj in enumerate(traj):
        ...
        results, d, v1, v2 = traj_analysis(n + 1, trj, box[n], system_info)
        output += results
        d_list.append(d)
        v1_list.append(v1)
        v2_list.append(v2)
    ...
    if dist.shape[1] > 0:
        np.savetxt('dist_length_all.dat', dist)

    vec1 = np.array(v1_list)
    if vec1.shape[1] > 0:
        np.savetxt('angle_vec_all.dat', vec1)

    vec2 = np.array(v2_list)
    if vec2.shape[1] > 0:
        np.savetxt('angel_vec_ads.dat')
...
def traj_analysis(idx, file, box, info):
    ...
    out, dist = compute_rdf(idx, coord, box, info)
    results += out
    ...
    print('\nAngle analysis\n')
    out, v1, v2 = compute_angle(idx, coord, record_coord, box, info)
    results += out

    return results, dist, v1, v2
