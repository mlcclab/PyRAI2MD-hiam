    ...
    with open('%s.freq.molden' % ld_input, 'r') as molden:
        ...
        for line in file:
            ...
            if """ [FR-COORD]""" in line:
                ...
                amass = [Element(atm).get_mass() for atm in atoms]
                amass = np.array(amass)
                achrg = [Element(atm).get_nuc() for atm in atoms]
...
def read_g16_fchk(ld_input):
    ...
    with open('%s.freq.fchk' % ld_input, 'r') as raw:
...
def read_orca(ld_input):
    ...
    for n, i in enumerate(freqs):
        if i > 0:
            ...
            print('imaginary frequency: %10f mode: %6d (Ignored)' % (i, n + 1))
    ...
    rmass = np.array([0 for _ in range(nmode)]).reshape((-1, 1))
    amass = [Element(atm).get_mass() for atm in atoms]
    amass = np.array(amass)
    amass = amass.reshape((natom, 1))
...
def read_bagel(ld_input):
    ...
    with open('%s.freq.bagel' % ld_input, 'r') as raw:
        log = raw.read().splitlines()
...
def load_newtonx(ld_input):
    ...
    with open('%s.init.newtonx' % ld_input, 'r') as initcond:
...
def load_xz(ld_input):
    ## This function read .init.tar.xz file and return all as a list of dict

    with tarfile.open('%s.init.tar.xz' % ld_input, 'r:xz') as initcond:
...
def boltzmann(sample):
    ...
    ## w          angular frequency           [s-1]            w=2*pi*nu
    ...
    ##
    ## sigma_Q = sqrt(kb*T/m)/w    standard deviation of position in Boltzmann distribution
    ...
    amass = np.array([np.ones(3) * i for i in amass])  # expand atomic mass over x y z coordinates
    ...
    q_p = np.array([[gaussian(), gaussian()] for _ in freqs])  # generates update coordinates and momenta pairs Q and P
    ...
    initcond = np.concatenate((atoms.reshape((-1, 1)), initcond), axis=1)
    initcond = np.concatenate((initcond, amass[:, 0: 1]), axis=1)
    initcond = np.concatenate((initcond, achrg), axis=1)
    return initcond
...
def wigner(sample):
    ...
    ## Q = sqrt(w*m/h_bar)*q => q = Q*sqrt(h_bar/w*m) = Q*sqrt(h_bar^2/w*h_bar*m)
    ##   = Q*sqrt(h_bar^2/Eh*me)/[ sqrt(E/Eh)*sqrt(m/me) ] = Q*a0/[ sqrt(mu*(h*c/Eh))*sqrt(ma/(Na*me*1000)) ]
    ...
    ##
    ## Convert velocity[m/s] to atomic unit[Bohr/au]
    ## P = p/sqrt(w*m*h_bar) => p = m*v = P*sqrt(w*m*h_bar) => v = P*sqrt(w*h_bar/m)
    ##   = P*sqrt(Eh/me)*sqrt(E/Eh)/sqrt(m/me) = P*vau*sqrt(mu*(h*c/Eh))/sqrt(ma/Na*me*1000))
    ## v = P*sqrt(mu*(h*c/Eh))/sqrt(ma/Na*me*1000)) in [Bohr/au]
    ## Update velocity from 0 to v
    ...
    # velo*=np.sqrt(amass*ma_to_amu)                        # only use for velo=2 in &DYNAMIX convert velocity[Bohr/au]
    # to mass-weighted [Bohr*amu^0.5/au] for Molcas

    initcond = np.concatenate((newc, velo), axis=1)
    initcond = np.concatenate((atoms.reshape((-1, 1)), initcond), axis=1)
    initcond = np.concatenate((initcond, amass), axis=1)
    initcond = np.concatenate((initcond, achrg), axis=1)

    return initcond


def sampling(ld_input, nesmb, iseed, temp, dist, ld_format):
    ## This function receives input information and does sampling
    ## This function use read_data to call different functions to extract vibrational frequency and mode
    ...
    if ld_format in callsample:
        sample = read_data[ld_format](ld_input)
        sample['temp'] = temp
        ...
        q = open('%s-%s-%s.xyz' % (dist, ld_input, temp), 'wb')
        p = open('%s-%s-%s.velocity' % (dist, ld_input, temp), 'wb')
        pq = open('%s.init' % ld_input, 'wb')
        m = 0
    ...
    elif ld_format in skipsample:
        ensemble = read_data[ld_format](ld_input)
        ...
        sys.exit('\n  TypeError\n  PyRAI2MD: freq file format %s was not recognized' % ld_format)

    return ensemble


def equilibrium(ld_input, ld_format):
    ## This function receives input information and read equilibrium geometry
    ## This function use read_data to call different functions to extract vibrational frequency and mode
    ...
    if ld_format in callsample:
        ...
        print('Nothing read from %s' % ld_input)
...
def main():
    ...
    parser.add_option('-i', dest='ld_input', type=str, nargs=1, help='Input freq.molden name.')
    parser.add_option('-n', dest='nesmb', type=int, nargs=1, help='Number of structure. Default is 1.', default=1)
    parser.add_option('-s', dest='iseed', type=int, nargs=1, help='Random number seed (0 - +inf). Default is random.',
                      default=-1)
    parser.add_option('-t', dest='temp', type=float, nargs=1, help='Temperature in K. Default is 298.15K.',
                      default=298.15)
    parser.add_option('-d', dest='dist', type=str, nargs=1,
                      help='Sampling method: wigner or boltzmann (low-cases). Default is wigner.', default='wigner')

    (options, args) = parser.parse_args()
    if options.ld_input is None:
        print(usage)
        exit()

    ld_input = options.ld_input.split('.')[0]
    ld_format = options.ld_input.split('.')[-1]
    iseed = options.iseed
    temp = options.temp
    dist = options.dist
    nesmb = options.nesmb

    ensemble = sampling(ld_input, nesmb, iseed, temp, dist, ld_format)[:]
